...
def topsis(input_file, weights, impacts, result_file):
        ...
        data = pd.read.csv(input_file)
        decision_matrix = data.iloc[:, 1:].values
        weights = np.array([float(w) for w in weights.split(',')])
        impacts = impacts.split(',')
    
# Normalize the decision matrix
normalized_matrix = decision_matrix / np.sqrt(( decision_matrix ** 2).sum(axis=0)) # type: ignore
# Calculate the weighted normalized matrix
weighted_normalized_matrix = normalized_matrix * weights
    # Determine the ideal and negative ideal solutions
ideal_solution = np.array([weighted_normalized_matrix[:, i].max() if impacts[i] == '+' else weighted_normalized_matrix[:, i].min() for i in range(len(weights))])
negative_ideal_solution = np.array([weighted_normalized_matrix[:, i].min() if impacts[i] == '+' else weighted_normalized_matrix[:, i].max() for i in range(len(weights))])
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 5:
        print("Usage: python topsis.py <weights> <impacts> <inputFileDataName> <resultFileDataName>")
        sys.exit(1)
